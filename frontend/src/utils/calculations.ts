...
export function calculateBeam(input: CalcInput): CalcResult {
  ...
  const ast = (0.5 * fck * b * d / fy) * (1 - Math.sqrt(1 - (4.6 * mu) / (fck * b * d * d)));
  const minimumAst = (0.85 / fy) * b * d * 100;
  const providedAst = Math.max(ast, minimumAst);
...
export function calculateFoundation(input: CalcInput): CalcResult {
  ...
  // Step 2: Depth
  const netUpwardPressure = (columnLoad * 1.5) / (size * size);
  const projection = (size * 1000 - columnWidth) / 2;
  const moment = (netUpwardPressure * projection * projection) / 2;
  const dRequired = Math.sqrt((moment * 1e6) / (0.138 * fck * 1000));
  ...
  steps.push({ description: 'Required Depth', value: `${dRequired.toFixed(0)} mm` });
  steps.push({ description: 'Overall Depth', value: `${overallDepth.toFixed(0)} mm` });
