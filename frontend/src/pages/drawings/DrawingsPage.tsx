...
import { saveDrawings, loadDrawings, deleteDrawing as deleteDrawingFromDB } from '@/services/drawingStorage';
...
export default function DrawingsPage() {
  ...
  const categories = ['All', ...new Set(drawings.map((d) => d.category).filter(Boolean))] as string[];

  // Load from IndexedDB
  useEffect(() => {
    loadDrawings()
      .then((loaded) => setDrawings(loaded))
      .catch((err) => console.error('Failed to load drawings:', err))
      .finally(() => setIsLoading(false));
  }, []);

  const persistDrawings = useCallback(async (updated: Drawing[]) => {
    try { await saveDrawings(updated); } catch (err) { console.error('Failed to save drawings:', err); }
  }, []);
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const newDrawings: Drawing[] = Array.from(files).map((file) => {
      const ext = file.name.split('.').pop()?.toUpperCase() || 'UNKNOWN';
      const today = new Date().toISOString().split('T')[0];
      const dataUrl = isImageType(ext) ? URL.createObjectURL(file) : undefined;
      return {
    ...
    const updated = [...newDrawings, ...drawings];
    setDrawings(updated);
  ...
  const handleDelete = async (id: string) => {
    ...
    if (d?.dataUrl) URL.revokeObjectURL(d.dataUrl);
    try { await deleteDrawingFromDB(id); } catch (err) { /* ignore */ }
  ...
  const handleDownload = (drawing: Drawing) => {
    if (drawing.file) {
      ...
      a.href = url; a.download = drawing.name;
  ...
  const addRateItem = (drawingId: string) => {
    ...
    updateDrawing(drawingId, { rateItems: [...(d.rateItems || []), newItem] });
  };

  const updateRateItem = (drawingId: string, itemId: string, updates: Partial<RateItem>) => {
    const d = drawings.find((x) => x.id === drawingId);
    if (!d) return;
    const items = (d.rateItems || []).map((item) =>
      item.id === itemId ? { ...item, ...updates } : item
    );
    updateDrawing(drawingId, { rateItems: items });
  };

  const removeRateItem = (drawingId: string, itemId: string) => {
    const d = drawings.find((x) => x.id === drawingId);
    if (!d) return;
    updateDrawing(drawingId, { rateItems: (d.rateItems || []).filter((x) => x.id !== itemId) });
  };
  ...
  // Calculate total rate for a drawing
  const getTotalRate = (d: Drawing) => {
    return (d.rateItems || []).reduce((sum, item) => sum + item.quantity * item.rate, 0);
  };

  // Stats
  const totalDrawings = drawings.length;
  const imageDrawings = drawings.filter((d) => isImageType(d.type)).length;
  const cadDrawings = drawings.filter((d) => ['DWG', 'DXF', 'DGN'].includes(d.type.toUpperCase())).length;
  const pdfDrawings = drawings.filter((d) => d.type.toUpperCase() === 'PDF').length;
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-[hsl(var(--muted-foreground))]">Store, preview, and analyze engineering drawings</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <Button onClick={handleUploadClick}><Upload size={16} className="mr-1" /> Upload Drawing</Button>
        </div>
      </div>
      ...
      {!isLoading && drawings.length === 0 && (
        <div className="text-center py-24 border-2 border-dashed border-[hsl(var(--border))] rounded-xl">
          <div className="inline-flex p-4 rounded-full bg-[hsl(var(--secondary))] mb-4">
            <Upload size={40} className="text-[hsl(var(--muted-foreground))]" />
          </div>
          ...
          <p className="text-[hsl(var(--muted-foreground))] max-w-md mx-auto mb-6">
            Upload your engineering drawings, sketches, and CAD files. They are stored securely in your browser and persist across sessions.
          </p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {filteredDrawings.map((d) => (
            <Card key={d.id} className="group overflow-hidden hover:shadow-lg transition-all duration-200 border-[hsl(var(--border))] hover:border-primary/30">
              ...
              >
                <div className={`aspect-[4/3] ${getCardBg(d.type)} flex items-center justify-center relative overflow-hidden`}>
                  {d.dataUrl && isImageType(d.type) ? (
                    <img src={d.dataUrl} alt={d.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
                  ) : (
                    getFileIcon(d.type, 48)
                  )}
                  ...
                  <div className="absolute top-2 left-2">
                    <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${getTypeColor(d.type)}`}>
                      {d.type}
                    </span>
                  </div>
              ...
              <CardContent className="p-3">
                <div className="flex items-start justify-between gap-2 mb-1">
                  <h3 className="font-medium text-sm truncate flex-1" title={d.name}>{d.name}</h3>
                </div>
                <div className="flex items-center gap-3 text-xs text-[hsl(var(--muted-foreground))]">
                  <span className="flex items-center gap-1"><Calendar size={11} /> {d.date}</span>
                  <span className="flex items-center gap-1"><HardDrive size={11} /> {d.size}</span>
                </div>
                ...
                <div className="mt-2 flex items-center gap-1">
                  <select
                    ...
                    onChange={(e) => { e.stopPropagation(); updateDrawing(d.id, { category: e.target.value }); }}
                      ...
                      {(d.rateItems || []).map((item) => (
                        <div key={item.id} className="flex items-center gap-1">
                          <input
                            placeholder="Item"
                            value={item.name}
                            onChange={(e) => updateRateItem(d.id, item.id, { name: e.target.value })}
                            className="flex-1 text-[10px] bg-[hsl(var(--secondary))] rounded px-1.5 py-0.5 border-0 outline-none min-w-0"
                          />
                          <input
                            type="number"
                            value={item.quantity || ''}
                            onChange={(e) => updateRateItem(d.id, item.id, { quantity: parseFloat(e.target.value) || 0 })}
                          ...
                          <input
                            type="number"
                            value={item.rate || ''}
                            onChange={(e) => updateRateItem(d.id, item.id, { rate: parseFloat(e.target.value) || 0 })}
                          ...
                          <Button variant="ghost" size="sm" className="h-4 w-4 p-0 text-red-400"
                            onClick={() => removeRateItem(d.id, item.id)}>
                            <X size={10} />
                          </Button>
                        </div>
                      ))}
          ...
          <div className="divide-y divide-[hsl(var(--border))]">
            ...
            {filteredDrawings.map((d) => (
              <div key={d.id} className="grid grid-cols-12 gap-3 px-4 py-3 items-center hover:bg-[hsl(var(--secondary))/30] transition-colors">
                ...
                <div className="col-span-1 text-sm text-[hsl(var(--muted-foreground))]">{d.size}</div>
                <div className="col-span-2 text-sm text-[hsl(var(--muted-foreground))]">{d.date}</div>
          ...
          <div className="bg-[hsl(var(--card))] rounded-xl max-w-5xl w-full max-h-[95vh] flex flex-col shadow-2xl border border-[hsl(var(--border))] overflow-hidden"
            ...
            <div className="flex items-center justify-between px-5 py-3 border-b border-[hsl(var(--border))] bg-[hsl(var(--card))]">
              <div className="flex items-center gap-3 min-w-0">
                <div className="p-2 rounded-lg bg-[hsl(var(--secondary))] shrink-0">
                  {getFileIcon(previewDrawing.type, 24)}
                </div>
                <div className="min-w-0">
                  <h2 className="font-semibold truncate">{previewDrawing.name}</h2>
                  <div className="flex items-center gap-2 text-xs text-[hsl(var(--muted-foreground))]">
                    <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded ${getTypeColor(previewDrawing.type)}`}>
                      {previewDrawing.type}
                    </span>
                    <span>{previewDrawing.size}</span>
                    <span>{previewDrawing.date}</span>
                  </div>
                </div>
              </div>
            ...
            <div className="flex-1 overflow-auto p-5 flex flex-col lg:flex-row gap-5 bg-[hsl(var(--secondary))/30]">
              ...
              <div className="flex-1 flex items-center justify-center min-h-[300px] bg-white dark:bg-gray-900 rounded-lg shadow-inner">
                {previewDrawing.dataUrl && isImageType(previewDrawing.type) ? (
                  <img src={previewDrawing.dataUrl} alt={previewDrawing.name}
                  ...
                  <div className="text-center py-16">
                    ...
                    <p className="text-[hsl(var(--muted-foreground))] text-sm max-w-md mb-4">
                      {previewDrawing.type === 'PDF' ? 'PDF files cannot be previewed directly. Download the file to view it.'
                        : ['DWG', 'DXF', 'DGN'].includes(previewDrawing.type)
                    ...
                    <div className="space-y-2 text-sm">
                      <div className="flex justify-between">
                        ...
                        <select value={previewDrawing.category || 'Other'}
                          onChange={(e) => {
                            updateDrawing(previewDrawing.id, { category: e.target.value });
                            setPreviewDrawing({ ...previewDrawing, category: e.target.value });
                          }}
                      ...
                      <div className="flex justify-between">
                        ...
                        <span className="font-medium">{previewDrawing.type}</span>
                      </div>
                      <div className="flex justify-between">
                  ...
                  <CardContent className="p-4 space-y-3">
                    <div className="flex items-center justify-between">
                      ...
                      <Button variant="ghost" size="sm" className="h-6 w-6 p-0"
                        onClick={() => addRateItem(previewDrawing.id)}>
                        <Plus size={14} />
                      </Button>
                    </div>
                    <div className="space-y-1.5">
                      {(previewDrawing.rateItems || []).map((item) => (
                        <div key={item.id} className="flex items-center gap-1">
                          <input placeholder="Item" value={item.name}
                            onChange={(e) => {
                              updateRateItem(previewDrawing.id, item.id, { name: e.target.value });
                              setPreviewDrawing({
                                ...previewDrawing,
                                rateItems: (previewDrawing.rateItems || []).map((ri) =>
                                  ri.id === item.id ? { ...ri, name: e.target.value } : ri
                                )
                              });
                            }}
                            className="flex-1 text-xs bg-[hsl(var(--secondary))] rounded px-1.5 py-1 border-0 outline-none" />
                          <input type="number" value={item.quantity || ''}
                            onChange={(e) => updateRateItem(previewDrawing.id, item.id, { quantity: parseFloat(e.target.value) || 0 })}
                            className="w-14 text-xs bg-[hsl(var(--secondary))] rounded px-1 py-1 border-0 outline-none text-right" placeholder="Qty" />
                          <input type="number" value={item.rate || ''}
                            onChange={(e) => updateRateItem(previewDrawing.id, item.id, { rate: parseFloat(e.target.value) || 0 })}
                          ...
                          <Button variant="ghost" size="sm" className="h-5 w-5 p-0 text-red-400"
                            onClick={() => removeRateItem(previewDrawing.id, item.id)}>
                            <X size={10} />
                          </Button>
                        </div>
                      ))}
